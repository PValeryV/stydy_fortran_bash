...
module List_IO
   ...
   type node
      type(node), allocatable  :: next, prev
   end type node
...
contains
   ...
   recursive subroutine Read_node_tree(In, tree)
      type(node), allocatable, intent(inout)  :: tree
        ...
        select case (char)
           case (CH__'+', CH__'-', CH__'*', CH__'/')
              if(.not. Allocated(tree))then
                 allocate(tree,operation(char=char))
              end if

 !             allocate (tree, source=operation(char=char))
               
           case (CH__'0':CH__'9')
              read (char, *) value
              allocate (tree, source=variable(value=value))
        end select
     else
        nullify (Elem)
     end if
