...
program reference_lab_2
   ...
   ! MALE = 4_"М"
   ! MALE = "М"
   type(student), allocatable  :: Group_List,Up_list,Before_list
   ...
   if (allocated(Group_List)) then
      call Output_class_list(output_file, Group_List, "Исходный список:", "rewind")
       ...
       if (List_len>0)then
          call Get_list_by_scrolling(Group_List, Before_list,List_len)
          call Output_class_list(output_file, Group_list, "Пролистывание","append") 

          call movement_list(Before_list,up_list,Group_list,Move_list,List_len,output_file)
       end if
   end if
